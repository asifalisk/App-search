...
class App extends Component{
  ...
  getactivetabid=searchinputs=>{
    const {activeTabId}=this.state
    const filterdtabs=searchinputs.filter(eachapp=>(
  ...
  searchinputs=()=>{
    ...
    const filteredsearch=appsList.filter(each =>(
  ...
  render(){
    const {inputvalue}=this.state
    const serached=this.searchinputs()
    const getactive=this.getactivetabid(serached)
    return (
      <div className='app'>
        <input type='search' value={inputvalue} className='input' onChange={this.onchangeinput} />
        <div className='tablist'  >
          {tabsList.map(eachtab=>(
            <Tablist setactivetab={this.setactivetabid} key={eachtab.tabId}  tablist={eachtab} />
          ))}
        </div>
        <div className='applist'>
          {getactive.map(eachapp =>(
            <Appstore applist={eachapp} key={eachapp.appId}/>
          ))}
